 ...
 *
 */
public final class IndexDetailFragment extends SecurityDetailFragment implements DetailActionBar.OnOperationListener,
        OperationBar.OnOperationListener, View.OnClickListener, ISecurityDetailView {
    private static final String TAG = IndexDetailFragment.class.getSimpleName();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        View contextView = getActivity().getLayoutInflater().inflate(R.layout.main_index_detail, container, false);
    ...
    private void setTimeStatHelper() {
        int marketType = StockUtil.getMarketType(mDtSecCode);
        switch (marketType) {
            case E_MARKET_TYPE.E_MT_SZ:
            case E_MARKET_TYPE.E_MT_SH:
                mTimeStatHelper.setKey(StatConsts.CHINA_INDEX);
                break;
            case E_MARKET_TYPE.E_MT_HIS:
            case E_MARKET_TYPE.E_MT_USI:
            default:
                mTimeStatHelper.setKey(StatConsts.OTHER_INDEX);
                break;
        }
    }
    ...
    protected TimeStatHelper createTimeStatHelper() {
        return new TimeStatHelper(BEACON_STAT_TYPE.E_BST_SEC_DETAIL_PAGE);
    }
    ...
    private void doOnResume() {
        ...
        switch (marketType) {
            ...
            case E_MARKET_TYPE.E_MT_USI:
            default:
                StatisticsUtil.reportAction(StatisticsConst.SECURITY_DETAIL_SHOW_OTHER_INDEX);
                break;
        }
        ...
        if (StockUtil.isHongKongIndex(mDtSecCode)) {
            showHKDelayPromptIfNeed();
        } else if(StockUtil.isUSAIndex(mDtSecCode)) {
            showUSADelayPromptIfNeed();
        }
    ...
    private void showHKDelayPromptIfNeed() {
        if (DataPref.isHongkongDelayPromptDisplayed()) {
            return;
        } else {
            DataPref.setHongkongDelayPromptDisplayed(true);
        }

        showDelayPromptIfNeed(R.string.hongkong_delay_prompt_msg);
    }

    private void showUSADelayPromptIfNeed() {
        ...
        showDelayPromptIfNeed(R.string.usa_delay_prompt_msg);
    }

    private void showDelayPromptIfNeed(int msg) {
    ...
    private void initViews() {
        ...
        if (StockUtil.hasStockRank(mDtSecCode)) {
            initRankFragment();
        } else {
            mContextView.findViewById(R.id.article_fragment_container).setVisibility(View.GONE);
        }
    }
    ...
    private void showSecTypeTitle() {
        ...
        if (StockUtil.isHongKongOrUsaIndex(mDtSecCode)) {
            tags.add(getString(R.string.hongkong_delay_actionbar_tag));
        }
    ...
    private void initArticleTabLayout(final FragmentManager fragmentManager, final TabLayout articleTabLayout, final TabLayout stickyLayout) {
        articleTabLayout.initWithTitles(
            new int[]{R.string.market_increase, R.string.market_decrease, R.string.market_turnover_rate},
            new String[]{FragmentFactory.RANK_UP, FragmentFactory.RANK_DOWN, FragmentFactory.RANK_TURNOVER_RATE});
    ...
    private void replaceFragmentByTag(FragmentManager fragmentManager, String tag, String currentTag, int containerId) {
        ...
        if (fragment == null) {
            ...
            if (fragment instanceof NewsFragment) {
                ...
                bundle = new Bundle();
                bundle.putString(CommonConst.KEY_SEC_CODE, mDtSecCode);
                int reqType = getReqTypeByTag(tag);
                bundle.putInt(RankFragment.KEY_REQ_TYPE, reqType);
                fragment.setArguments(bundle);
                mTabFragmentList.add(fragment);
            }
            if (!fragment.isAdded()) {
    ...
    private int getReqTypeByTag(String tag) {
        if (FragmentFactory.RANK_UP.equals(tag)) {
            return E_INDEX_STOCKS_REQ_TYPE.E_UP_RATE_REQ;
        } else if (FragmentFactory.RANK_DOWN.equals(tag)) {
            return E_INDEX_STOCKS_REQ_TYPE.E_DOWN_RATE_REQ;
        } else if (FragmentFactory.RANK_TURNOVER_RATE.equals(tag)) {
            return E_INDEX_STOCKS_REQ_TYPE.E_TURNOVER_RATE_REQ;
        }
        return E_INDEX_STOCKS_REQ_TYPE.E_UP_RATE_REQ;
    }
    ...
    private void switchTabIfNeeded(final boolean force) {
        String tabTag = DataPref.getIndexTabTag();
        final TabLayout articleTabLayout = (TabLayout) mContextView.findViewById(R.id.article_tabs);
