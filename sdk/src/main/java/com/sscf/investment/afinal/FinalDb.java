 ...
 */
package com.sscf.investment.afinal;
...
import com.sscf.investment.afinal.db.sqlite.CursorUtils;
import com.sscf.investment.afinal.db.sqlite.DbModel;
import com.sscf.investment.afinal.db.sqlite.ManyToOneLazyLoader;
import com.sscf.investment.afinal.db.sqlite.OneToManyLazyLoader;
import com.sscf.investment.afinal.db.sqlite.SqlBuilder;
import com.sscf.investment.afinal.db.sqlite.SqlInfo;
import com.sscf.investment.afinal.db.table.KeyValue;
import com.sscf.investment.afinal.db.table.ManyToOne;
import com.sscf.investment.afinal.db.table.OneToMany;
import com.sscf.investment.afinal.db.table.TableInfo;
import com.sscf.investment.afinal.exception.DbException;
...
public class FinalDb {
	...
	public boolean saveBindId(Object entity) {
		...
		if (entityKvList != null && entityKvList.size() > 0) {
			...
			long id = -1;
			if (db != null) {
				id = db.insert(tf.getTableName(), null, cv);
			}
	...
	public void dropDb() {
		...
		if (cursor != null) {
			while (cursor.moveToNext()) {
				execSQL("DROP TABLE " + cursor.getString(0));
			}
		}
		if (cursor != null) {
			cursor.close();
	...
	private void exeSqlInfo(SqlInfo sqlInfo) {
		if (sqlInfo != null) {
			final String sql = sqlInfo.getSql();
			debugSql(sql);
			if (db == null) {
				createDb();
			}
			if (db != null) {
				db.execSQL(sql, sqlInfo.getBindArgsAsArray());
			}
		} else {
			DtLog.e(TAG, "sava error:sqlInfo is null");
		}
	}
