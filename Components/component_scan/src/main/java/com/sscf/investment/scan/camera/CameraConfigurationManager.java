...
final class CameraConfigurationManager {
    ...
    private static final int TEN_DESIRED_ZOOM = 167; // 默认27
    ...
    void setDesiredCameraParameters(Camera camera) {
        ...
        final Camera.Size size = getMaxSupportedPictureSize(parameters);
        if (size == null) {
    ...
    private Camera.Size getMaxSupportedPictureSize(final Camera.Parameters parameters) {
        final List<Camera.Size> sizes = parameters.getSupportedPictureSizes();
        final int size = sizes == null ? 0 : sizes.size();
        if (size <= 0) {
            return null;
        } else if (size == 1) {
            return sizes.get(0);
        } else {
            final Camera.Size size1 = sizes.get(0);
            final Camera.Size size2 = sizes.get(size - 1);
...
//                    tenDesiredZoom = tenMaxZoom;
                    tenDesiredZoom = (int) (tenMaxZoom * 0.5); // 处理酷派手机焦距的问题
                }
            } catch (NumberFormatException nfe) {
        ...
        Log.w(TAG, "takingPictureZoomMaxString: " + takingPictureZoomMaxString);
        Log.w(TAG, "parameters.get(\"taking-picture-zoom\"): " + parameters.get("taking-picture-zoom"));
    }
}
