...
import com.sscf.investment.scan.CaptureActivity;
import com.sscf.investment.scan.camera.CameraManager;
import com.sscf.investment.scan.view.ViewfinderResultPointCallback;
...
public final class CaptureActivityHandler extends Handler {
  ...
  public void handleMessage(Message message) {
    ...
    if (what == R.id.auto_focus) {
      ...
      if (state == State.PREVIEW || state == State.PAUSE) {
    ...
    } else if (what == R.id.pause_decode) {
      state = State.PAUSE;
      CameraManager.get().removePreviewCallback();
      CameraManager.get().requestAutoFocus(this, R.id.auto_focus);
    }
  }
  ...
  private void restartPreviewAndDecode() {
    if (state == State.SUCCESS || state == State.PAUSE) {
      state = State.PREVIEW;
      CameraManager.get().requestPreviewFrame(decodeThread.getHandler(), R.id.decode);
      CameraManager.get().requestAutoFocus(this, R.id.auto_focus);
